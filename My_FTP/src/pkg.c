...
/*	构建指定数据包	 
	命令格式:0xc0 pkg_len cmd_no arg_1_len  arg_1_data 0xa0
...
uchar* pkg_resp(const uchar* content,int* len,enum CMD_NO mode)
{
	...
	cmd[i++] =  result;
	strncpy((char*)(cmd+i),(const char*)(content),x);
	i+=x;
	cmd[i++]=0xa0;
	uchar* buf = cmd;
	*len = i;
...
pkgInfo_rqst handle_pkg_rqst(uchar recv_buf[],int sockfd,int* len)
{
	...
	if (pkg.pkg_start != 0xc0)
	{
		...
		pkg.arg_1_len = (recv_buf[i++]) |
						...
						(recv_buf[i++] << 16) |
						(recv_buf[i++] << 24);
		strncpy((char *)pkg.arg_1_data, (const char *)(recv_buf + i), pkg.arg_1_len);
		i += pkg.arg_1_len;
	//包尾
